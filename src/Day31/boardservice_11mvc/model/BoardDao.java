...
public class BoardDao { // 주로 싱글톤
    ...
    public ArrayList<BoardDto> boardPrint() {   // main s
        ...
        try {
            ...
            while (rs.next()) { // while(조건){} : 반복문, rs.next() : 조회결과에서 다음 레코드 이동함수
                ...
                BoardDto boardDto = new BoardDto(num, content, writer);     // 게시물 번호는 과제로 생각해보기
    ...
    public boolean boardDelete(int deleteNum) {
        ...
        try {
            ...
            ps.setInt(1, deleteNum);    // 기재된 sql 내 첫번째 ?에 'deleteNum' 삭제할 게시물 번호를 대입한다
            // 4. SQL 실행
            int result = ps.executeUpdate();    // delete 실행 후 delete 레코드 개수를 번환, result(삭제된 레코드수)
            // 5. SQL 실행 결과
            // 6. 메소드 반환
            if (result == 1) {
                return true;    // 삭제 성공
    ...
    public boolean boardUpdate(BoardDto updateDto) {
        ...
        try {
            ...
            // 4. 기재된 SQL 실행
            int result = ps.executeUpdate();        // 수정 SQL, 결과 변화가 있는 렠코드수 반환
            // 5. 실행결과

            // 6. 메소드 반환
            if (result == 1) {  // 만약 레코드 변화가 1개 있으면 수정 성공
                return true;
            }
